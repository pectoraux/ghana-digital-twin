      ...
      <header className="flex h-14 shrink-0 items-center gap-3 border-b border-border bg-card/40 px-4">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col leading-tight">
            ...
            <span className="text-[9px] text-muted-foreground">{role.replace(/_/g, " ").toLowerCase()}</span>
          </div>
        </div>
        ...
        {rep && (
          <div className="ml-4 flex items-center gap-3">
            <div className="flex items-center gap-1">
              ...
              <span className="text-[11px] font-mono font-semibold text-cyan-400">{rep.trustScore.toFixed(0)}</span>
            </div>
            <div className="flex items-center gap-1">
              <Award className="size-3 text-amber-400" />
              <span className="text-[11px] font-mono font-semibold text-amber-400">{rep.civicScore.toFixed(0)}</span>
            </div>
          </div>
        )}
        {org && (
          <div className="ml-2 hidden md:flex items-center gap-1">
            ...
            <span className="text-[10px] text-muted-foreground">{org.orgName}</span>
          </div>
        )}
        <button onClick={() => signOut({ callbackUrl: "/login" })} className="ml-auto flex items-center gap-1 text-[10px] text-muted-foreground hover:text-foreground">
  ...
  return (
    <div className="h-full overflow-y-auto gdt-scroll">
      ...
      <div className="px-4 py-3 border-b border-border">
        ...
        <p className="text-[11px] text-muted-foreground">Your region: {region}</p>
      </div>
      ...
      <div className="flex items-center gap-1 px-4 py-2 overflow-x-auto border-b border-border">
        {["", "ALERT", "REPORT", "ANALYSIS", "MISSION", "ASSET", "ANNOUNCEMENT"].map((t) => (
          <button key={t || "all"} onClick={() => setFilter(t)}
            className={cn("rounded-full px-2.5 py-1 text-[10px] font-medium whitespace-nowrap transition-colors",
          ...
          return (
            <div key={item.feedItemId} className="rounded-lg border border-border bg-card/40 p-3">
              <div className="flex items-center gap-2 mb-1.5">
                <div className="size-7 rounded-full flex items-center justify-center text-[10px] font-bold" style={{ color: meta.color, background: `${meta.color}1a` }}>
                  {item.creatorName.charAt(0).toUpperCase()}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    <span className="text-[11px] font-medium truncate">{item.creatorName}</span>
                  ...
                  <div className="text-[9px] text-muted-foreground">{item.creatorRole.replace(/_/g, " ").toLowerCase()} · {timeAgo(item.publishedAt)}</div>
                </div>
                {item.organizationName && (
                  <span className="flex items-center gap-1 text-[9px] text-violet-400"><Building2 className="size-2.5" /> {item.organizationName}</span>
                )}
              </div>
              <h3 className="text-[13px] font-semibold mb-1">{item.title}</h3>
              <p className="text-[11px] text-foreground/70 leading-snug mb-2">{item.summary}</p>
  ...
  return (
    <div className="h-full overflow-y-auto gdt-scroll p-4">
      ...
      <div className="space-y-4">
        <div>
          <label className="text-[11px] font-medium text-muted-foreground mb-2 block">What happened?</label>
          <div className="grid grid-cols-3 gap-2">
            {categories.map((c) => (
              <button key={c.value} onClick={() => setCategory(c.value)}
                ...
                <span className="text-[9px] font-medium text-center">{c.label}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="text-[11px] font-medium text-muted-foreground mb-1.5 block">Where?</label>
          <div className="flex items-center gap-2 rounded-lg border border-border bg-card/30 px-3 py-2">
            <MapPin className="size-4 text-primary" />
            <span className="text-[11px] text-muted-foreground">GPS detected · {region ?? "Current location"}</span>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3}
            ...
            className="w-full rounded-lg border border-border bg-card/30 px-3 py-2 text-[12px] resize-none" />
        </div>

        <div>
          <label className="text-[11px] font-medium text-muted-foreground mb-1.5 block">Evidence (optional)</label>
          <div className="flex gap-2">
            <button className="flex items-center gap-1 rounded-lg border border-border bg-card/30 px-3 py-2 text-[10px] text-muted-foreground hover:bg-card/50">
              📷 Photo
            </button>
            <button className="flex items-center gap-1 rounded-lg border border-border bg-card/30 px-3 py-2 text-[10px] text-muted-foreground hover:bg-card/50">
              🎥 Video
  ...
  return (
    <div className="h-full overflow-y-auto gdt-scroll">
      <div className="px-4 py-3 border-b border-border">
        ...
        <p className="text-[11px] text-muted-foreground">Active intelligence alerts + witness requests</p>
      </div>
      <div className="p-3 space-y-2">
        {loading && <div className="flex h-32 items-center justify-center"><Loader2 className="size-5 animate-spin text-primary" /></div>}
        {alerts.map((a) => {
          ...
          return (
            <div key={a.feedItemId} className="rounded-lg border p-3" style={{ borderColor: `${meta.color}40`, background: `${meta.color}08` }}>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[12px] font-semibold flex-1">{a.title}</span>
                <span className="text-[9px] text-muted-foreground">{timeAgo(a.publishedAt)}</span>
              </div>
              <p className="text-[11px] text-foreground/70 mb-2">{a.summary}</p>
              <div className="flex items-center gap-2">
                <button className="flex items-center gap-1 rounded border border-emerald-500/20 bg-emerald-500/5 px-2 py-1 text-[10px] text-emerald-400">
                  <CheckCircle2 className="size-3" /> Verify
                </button>
                <button className="flex items-center gap-1 rounded border border-border bg-card/30 px-2 py-1 text-[10px] text-muted-foreground">
                  <Eye className="size-3" /> View
                </button>
                <span className="ml-auto text-[9px] text-muted-foreground">{a.region}</span>
              </div>
            </div>
          );
        })}
  ...
  return (
    <div className="h-full overflow-y-auto gdt-scroll p-4 space-y-4">
      ...
      <div className="rounded-lg border border-border bg-card/40 p-4 text-center">
        ...
        <p className="text-[11px] text-muted-foreground">{(user as any)?.role?.replace(/_/g, " ").toLowerCase()}</p>
        {org && <p className="text-[10px] text-violet-400 mt-1">{org.orgName}</p>}
        <div className="flex items-center justify-center gap-1 mt-1">
          <MapPin className="size-3 text-muted-foreground" />
          <span className="text-[10px] text-muted-foreground">{profile?.region ?? "Unknown"}, {profile?.country ?? "Ghana"}</span>
        </div>
      </div>
      ...
      {rep && (
        <div className="rounded-lg border border-border bg-card/40 p-4">
          <h3 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-3">Reputation</h3>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="text-center">
              ...
              <div className="text-[9px] text-muted-foreground">Civic Score</div>
            </div>
          </div>
          <div className="mt-3 flex items-center justify-center gap-2">
            <span className="rounded bg-foreground/5 px-2 py-0.5 text-[9px] font-medium">{rep.verificationLevel}</span>
            <span className="rounded bg-foreground/5 px-2 py-0.5 text-[9px]">contribution: {rep.contributionScore}</span>
          </div>
        </div>
      )}
      ...
      {rep && (
        <div className="rounded-lg border border-border bg-card/40 p-4">
          <h3 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-3">Impact</h3>
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded bg-card/30 px-3 py-2">
              ...
              <div className="text-[9px] text-muted-foreground">Reports Filed</div>
            </div>
            <div className="rounded bg-card/30 px-3 py-2">
            ...
            <div className="rounded bg-card/30 px-3 py-2">
              ...
              <div className="text-[9px] text-muted-foreground">Missions</div>
            </div>
          </div>
        </div>
      )}
      ...
      {profile?.bio && (
        <div className="rounded-lg border border-border bg-card/40 p-4">
          ...
          <p className="text-[11px] text-foreground/80 leading-relaxed">{profile.bio}</p>
          {profile.interests?.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {profile.interests.map((i: string) => (
                <span key={i} className="rounded bg-foreground/5 px-1.5 py-0.5 text-[9px]">{i}</span>
              ))}
            </div>
          )}
        </div>
