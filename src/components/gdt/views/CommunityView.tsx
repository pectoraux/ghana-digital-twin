    ...
    <div className="h-full flex">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="px-4 py-1.5 border-b border-border flex items-center gap-1">
          <span className="text-[14px] text-muted-foreground mr-1">Status:</span>
          {["", "witnessing", "verified", "resolved", "learned"].map((s) => (
            <button key={s || "all"} onClick={() => setFilter({ status: s || undefined })}
              className={cn("rounded px-1.5 py-0.5 text-[14px] font-medium capitalize transition-colors",
              ...
              >
                <div className="flex items-stretch gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[16px] font-medium mb-0.5 truncate">{ev.title}</div>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="px-3 py-2 border-b border-border">
        ...
        <div className="text-[16px] font-medium">{ev.title}</div>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto gdt-scroll p-3 space-y-3">
        {/* Description */}
        <div>
          <SectionLabel className="mb-1">Claim</SectionLabel>
          <p className="text-[15px] text-foreground/80 leading-relaxed">{ev.description}</p>
        </div>
              ...
              return (
                <div key={w.responseId} className="rounded border border-border bg-card/30 px-2 py-1.5">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <span className="font-mono text-[14px] font-medium">{w.witnessId}</span>
                    <span className="text-[13px] rounded px-1 py-0.5" style={{ color, background: `${color}1a` }}>{w.response}</span>
                    <span className="ml-auto text-[13px] text-muted-foreground">civic {w.witnessCivicScore}</span>
                  </div>
                  {w.note && <div className="text-[14px] text-foreground/70">{w.note}</div>}
                  {w.distanceToEventM && <div className="text-[13px] text-muted-foreground mt-0.5">{(w.distanceToEventM / 1000).toFixed(1)}km away</div>}
                </div>
              );
            })}
            {witnesses.length === 0 && <div className="text-[14px] text-muted-foreground italic">No witness responses yet</div>}
          </div>
        </div>
        ...
        {ev.outcome && (
          <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-2.5">
            ...
            <div className="text-[15px] font-semibold text-emerald-300 mb-0.5">{ev.outcome}</div>
            {ev.resolutionNote && <div className="text-[14px] text-foreground/70">{ev.resolutionNote}</div>}
            {ev.rewardDistributed && <div className="text-[13px] text-amber-400 mt-1 flex items-center gap-1"><DollarSign className="size-2.5" /> Reward distributed</div>}
          </div>
        )}
          ...
          <div className="space-y-1.5">
            {ev.fusedConfidence >= 0.5 && !ev.fusedIncidentId && (
              <button onClick={handleFuse} disabled={busy === "fuse"}
                className="w-full flex items-center justify-center gap-1.5 rounded border border-primary/30 bg-primary/10 px-2 py-1.5 text-[15px] text-primary hover:bg-primary/15 disabled:opacity-50">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              <button onClick={() => handleResolve("confirmed")} disabled={!!busy}
                className="flex items-center justify-center gap-1 rounded border border-emerald-500/20 bg-emerald-500/5 px-2 py-1.5 text-[14px] text-emerald-400 hover:bg-emerald-500/10 disabled:opacity-50">
              ...
              <button onClick={() => handleResolve("false_positive")} disabled={!!busy}
                className="flex items-center justify-center gap-1 rounded border border-rose-500/20 bg-rose-500/5 px-2 py-1.5 text-[14px] text-rose-400 hover:bg-rose-500/10 disabled:opacity-50">
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 py-2 border-b border-border flex items-center gap-2 flex-wrap">
        ...
        <span className="text-[15px] text-muted-foreground">— the Waze mechanism: nearby citizens validate reports</span>
        <select
          value={citizenId}
          onChange={(e) => setCitizenId(e.target.value)}
          className="ml-auto rounded border border-border bg-card px-2 py-1 text-[15px]"
        >
          {citizens.map((c) => <option key={c.citizenId} value={c.citizenId}>{c.handle} ({c.civicScore})</option>)}
        </select>
        <button onClick={loadQueue} className="flex items-center gap-1 rounded border border-border px-2 py-1 text-[14px] text-muted-foreground hover:text-foreground">
              ...
              <div className="flex items-stretch gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="font-mono text-[14px] font-semibold text-muted-foreground">{ev.eventId}</span>
                    <span className="rounded px-1 py-0.5 text-[13px] font-semibold" style={{ color: tmeta.color, background: `${tmeta.color}1a` }}>{tmeta.label}</span>
                    <span className="rounded px-1 py-0.5 text-[13px] font-semibold uppercase" style={{ color: smeta.color, background: smeta.bg }}>{ev.severity}</span>
                  </div>
                  <div className="text-[16px] font-medium mb-0.5">{ev.title}</div>
                  <p className="text-[15px] text-foreground/70 leading-snug line-clamp-2 mb-1.5">{ev.description}</p>
                  <div className="flex items-center gap-3 text-[14px] text-muted-foreground mb-2">
                    <span className="flex items-center gap-1"><MapPin className="size-2.5" /> {ev.regionId ?? "—"}</span>
                  ...
                  <div className="flex items-center gap-1.5">
                    <button onClick={() => handleWitness(ev.eventId, "confirm")} disabled={!!busy}
                      className="flex items-center gap-1 rounded border border-emerald-500/20 bg-emerald-500/5 px-2 py-1 text-[14px] text-emerald-400 hover:bg-emerald-500/10 disabled:opacity-50">
                    ...
                    <button onClick={() => handleWitness(ev.eventId, "reject")} disabled={!!busy}
                      className="flex items-center gap-1 rounded border border-rose-500/20 bg-rose-500/5 px-2 py-1 text-[14px] text-rose-400 hover:bg-rose-500/10 disabled:opacity-50">
                    ...
                    <button onClick={() => handleWitness(ev.eventId, "unknown")} disabled={!!busy}
                      className="flex items-center gap-1 rounded border border-border bg-card/30 px-2 py-1 text-[14px] text-muted-foreground hover:text-foreground disabled:opacity-50">
    ...
    <div className="h-full flex">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="px-4 py-2 border-b border-border flex items-center gap-2">
          ...
          <span className="text-[15px] text-muted-foreground">— ranked by Civic Score (prediction accuracy, not popularity)</span>
          <span className="ml-auto text-[14px] text-muted-foreground">{citizens.length} citizens</span>
        </div>
        <div className="min-h-0 flex-1 overflow-y-auto gdt-scroll p-3 space-y-1.5">
          {loading && <div className="flex h-32 items-center justify-center"><Loader2 className="size-5 animate-spin text-primary" /></div>}
          {citizens.map((c, i) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-[16px] font-medium truncate">{c.handle}</span>
                      <span className="rounded px-1 py-0.5 text-[13px] font-semibold" style={{ color: trust.color, background: `${trust.color}1a` }}>{trust.label}</span>
                      <span className="font-mono text-[14px] text-muted-foreground">{c.citizenId}</span>
                    </div>
...
function CivicScoreDetail({ citizenId, score }: { citizenId: string; score: any }) {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="px-3 py-2 border-b border-border">
        <div className="text-[16px] font-semibold">{citizen.handle}</div>
        <div className="font-mono text-[14px] text-muted-foreground">{citizen.citizenId}</div>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto gdt-scroll p-3 space-y-3">
        {/* Overall score */}
        <div className="rounded-lg border border-border bg-card/40 p-3 text-center">
          <div className="text-[14px] uppercase tracking-wider text-muted-foreground mb-1">Civic Score</div>
          <div className="text-[36px] font-bold font-mono" style={{ color: citizen.civicScore >= 65 ? "#34d399" : citizen.civicScore >= 50 ? "#fbbf24" : "#fb923c" }}>
            {citizen.civicScore}
          </div>
          <div className="text-[14px] text-muted-foreground">/ 100 · {TRUST_META[citizen.trustLevel]?.label ?? citizen.trustLevel}</div>
          {score?.trendDelta !== null && score?.trendDelta !== undefined && (
            <div className={cn("text-[14px] mt-1 flex items-center justify-center gap-1", score.trendDelta >= 0 ? "text-emerald-400" : "text-rose-400")}>
                ...
                return (
                  <div key={key} className="rounded border border-border bg-card/30 px-2 py-1.5">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="font-mono text-[14px] font-semibold" style={{ color }}>{(value * 100).toFixed(0)}%</span>
                      <span className="text-[13px] text-muted-foreground">w {(weight * 100).toFixed(0)}%</span>
                    </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="rounded border border-border bg-card/30 px-2 py-1.5">
            <div className="text-[13px] text-muted-foreground">Reports</div>
            <div className="font-mono text-[18px] font-semibold">{citizen.totalReports}</div>
          </div>
          <div className="rounded border border-border bg-card/30 px-2 py-1.5">
            <div className="text-[13px] text-muted-foreground">Confirmed</div>
            <div className="font-mono text-[18px] font-semibold text-emerald-400">{citizen.confirmedReports}</div>
          </div>
          <div className="rounded border border-border bg-card/30 px-2 py-1.5">
            <div className="text-[13px] text-muted-foreground">False</div>
            <div className="font-mono text-[18px] font-semibold text-rose-400">{citizen.falseReports}</div>
          </div>
          <div className="rounded border border-border bg-card/30 px-2 py-1.5">
            <div className="text-[13px] text-muted-foreground">Earnings</div>
            <div className="font-mono text-[18px] font-semibold text-amber-400">${citizen.totalEarnings.toFixed(2)}</div>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 py-2 border-b border-border flex items-center gap-2">
        ...
        <span className="text-[15px] text-muted-foreground">— bounty pools + contributor payments</span>
        <div className="ml-auto flex items-center gap-3 text-[14px] text-muted-foreground">
            ...
            {data.pools.map((p) => {
              ...
              return (
                <div key={p.poolId} className="rounded-lg border border-border bg-card/40 p-3">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="size-6 rounded-md flex items-center justify-center font-bold text-[14px]" style={{ color: funderColor, background: `${funderColor}1a` }}>
                      {p.funderName.slice(0, 2).toUpperCase()}
                    </span>
                    <div className="min-w-0 flex-1">
                      <div className="text-[16px] font-semibold truncate">{p.name}</div>
                      <div className="text-[13px] text-muted-foreground">{p.funderName} · {p.funder}</div>
                    </div>
                    <span className="rounded px-1 py-0.5 text-[13px] font-semibold" style={{ color: funderColor, background: `${funderColor}1a` }}>
                      {TYPE_META[p.category]?.label ?? p.category}
                    </span>
                  </div>
                  <p className="text-[14px] text-muted-foreground mb-2 leading-snug">{p.description}</p>
                  <div className="flex items-center gap-3 text-[14px] mb-1.5">
        ...
        <div>
          ...
          <div className="space-y-1.5">
            {data.distributions.map((d) => (
              <div key={d.distributionId} className="rounded-lg border border-border bg-card/40 p-2.5">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="font-mono text-[14px] font-semibold text-amber-300">{d.distributionId}</span>
                  <span className="text-[14px] text-muted-foreground">→ {d.eventId}</span>
                  <span className="ml-auto font-mono text-[18px] font-bold text-amber-400">${d.totalAmount.toFixed(2)}</span>
                </div>
                ...
                <div className="mt-1.5 flex items-center gap-1 flex-wrap">
                  <span className="text-[13px] text-muted-foreground">Recipients:</span>
                  {Object.entries(d.recipientBreakdown).map(([id, amount]: [string, any]) => (
                    id !== "platform" && (
                      <span key={id} className="rounded bg-foreground/5 px-1 py-0.5 text-[13px] font-mono">
            ...
            {data.distributions.length === 0 && !loading && (
              <div className="text-[15px] text-muted-foreground italic">No reward distributions yet</div>
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 py-2 border-b border-border flex items-center gap-2">
        ...
        <span className="text-[15px] text-muted-foreground">— users subscribe to region/category/risk → get alerts on verified events</span>
        <span className="ml-auto text-[14px] text-muted-foreground">{subs.length} subscriptions</span>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto gdt-scroll p-3 space-y-1.5">
        {loading && <div className="flex h-32 items-center justify-center"><Loader2 className="size-5 animate-spin text-primary" /></div>}
        {subs.map((s) => (
          <div key={s.subscriptionId} className="rounded-lg border border-border bg-card/40 px-3 py-2.5">
            <div className="flex items-center gap-2 mb-1.5">
              <span className="font-mono text-[14px] font-semibold text-muted-foreground">{s.subscriptionId}</span>
              <span className="text-[15px] font-medium">{s.citizenId}</span>
              {s.alertCount > 0 && (
                <span className="flex items-center gap-1 rounded bg-cyan-500/10 px-1 py-0.5 text-[13px] font-semibold text-cyan-400">
                  <Bell className="size-2.5" /> {s.alertCount} alerts
                </span>
              )}
              <span className="ml-auto text-[13px] text-muted-foreground">{s.active ? "active" : "inactive"}</span>
            </div>
            <div className="flex items-center gap-2 text-[14px] flex-wrap">
              ...
              {s.categories.length > 0 ? (
                s.categories.map((c: string) => (
                  <span key={c} className="rounded px-1 py-0.5 text-[13px] font-semibold" style={{ color: TYPE_META[c]?.color ?? "#a1a1aa", background: `${TYPE_META[c]?.color ?? "#a1a1aa"}1a` }}>
                    {TYPE_META[c]?.label ?? c}
                  </span>
                ))
              ) : (
            ...
            {s.lastAlertAt && (
              <div className="text-[13px] text-muted-foreground mt-1">Last alert: {timeAgo(s.lastAlertAt)}</div>
            )}
          </div>
        ))}
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 py-2 border-b border-border flex items-center gap-2 flex-wrap">
        ...
        <span className="text-[15px] text-muted-foreground">— unified: agents + citizens + sensors + organizations</span>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto gdt-scroll p-3 space-y-1.5">
          ...
          return (
            <div key={p.producerId} className="rounded-lg border border-border bg-card/40 px-3 py-2.5">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-[16px] font-medium truncate">{p.name}</span>
                    ...
                    <span className="rounded px-1 py-0.5 text-[13px] font-semibold" style={{ color: trust.color, background: `${trust.color}1a` }}>{trust.label}</span>
                    <span className="font-mono text-[14px] text-muted-foreground">{p.producerId}</span>
                  </div>
                  <div className="flex items-center gap-3 text-[14px] text-muted-foreground">
                ...
                <div className="text-right shrink-0">
                  <div className="text-[18px] font-bold font-mono" style={{ color: pct >= 70 ? "#34d399" : pct >= 50 ? "#fbbf24" : "#fb923c" }}>
                    {pct.toFixed(0)}<span className="text-[14px] text-muted-foreground">%</span>
                  </div>
                  <div className="text-[13px] text-muted-foreground">reputation</div>
                </div>
              </div>
            </div>
          );
        ...
        <div className="rounded-lg border border-primary/20 bg-primary/5 p-3.5 mt-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[15px] font-semibold text-primary">UNIFIED REPUTATION GRAPH</span>
          </div>
