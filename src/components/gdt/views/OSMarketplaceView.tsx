    ...
    <div className="h-full overflow-y-auto gdt-scroll p-4 space-y-4">
        ...
        <div className="rounded-lg border border-border bg-card/40 p-4">
          ...
          <div className="space-y-1">
            {o.packagesByStage.map((s: any) => {
              ...
              return (
                <div key={s.lifecycleStage} className="flex items-center gap-2 text-[15px]">
        ...
        <div className="rounded-lg border border-border bg-card/40 p-4">
          ...
          <div className="space-y-1">
            {o.topPackages.map((p: any, i: number) => {
              ...
              return (
                <div key={p.packageId} className="flex items-center gap-2 text-[15px] py-0.5">
                  <span className="font-mono text-[16px] font-bold w-5 text-center" style={{ color: i < 3 ? "#fbbf24" : "#71717a" }}>#{i + 1}</span>
        ...
        <div className="rounded-lg border border-border bg-card/40 p-4">
          ...
          <div className="space-y-1">
            {o.topDevelopers.map((d: any, i: number) => (
              <div key={d.developerId} className="flex items-center gap-2 text-[15px] py-0.5">
                <span className="font-mono text-[16px] font-bold w-5 text-center" style={{ color: i < 3 ? "#fbbf24" : "#71717a" }}>#{i + 1}</span>
        ...
        <div className="rounded-lg border border-border bg-card/40 p-4">
          ...
          <div className="space-y-1">
            {o.topSolutions.map((s: any) => (
              <div key={s.solutionId} className="flex items-center gap-2 text-[15px] py-0.5">
      ...
      <div className="rounded-lg border border-primary/20 bg-primary/5 p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[15px] font-semibold text-primary">INTELLIGENCE OS MARKETPLACE</span>
        </div>
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 py-2 border-b border-border flex items-center gap-2 flex-wrap">
        ...
        <span className="ml-auto text-[14px] text-muted-foreground">{packages.length} packages</span>
      </div>
              ...
              <div className="flex items-stretch gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <span className="font-mono text-[14px] font-semibold text-muted-foreground">{p.packageId}</span>
                    ...
                    <span className="rounded px-1 py-0.5 text-[13px] font-semibold" style={{ color: stageMeta.color, background: `${stageMeta.color}1a` }}>{stageMeta.label}</span>
                    {p.securityReviewed && <span className="rounded bg-emerald-500/10 px-1 py-0.5 text-[13px] text-emerald-400"><ShieldCheck className="size-2.5 inline" /> security</span>}
                    {p.conformancePassed && <span className="rounded bg-cyan-500/10 px-1 py-0.5 text-[13px] text-cyan-400"><CheckCircle2 className="size-2.5 inline" /> conformance</span>}
                    <span className="ml-auto text-[13px] text-muted-foreground">v{p.version}</span>
                  </div>
                  <div className="text-[16px] font-medium mb-0.5">{p.displayName}</div>
                  ...
                  {p.capabilities.length > 0 && (
                    <div className="flex items-center gap-1 flex-wrap">
                      <span className="text-[13px] text-muted-foreground">capabilities:</span>
                      {p.capabilities.map((c: string) => (
                        <span key={c} className="rounded bg-foreground/5 px-1 py-0.5 text-[13px] font-mono">{c}</span>
                      ))}
                    </div>
                  )}
                  {/* Pricing */}
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[18px] font-bold font-mono" style={{ color: p.pricePerUse === 0 && p.subscriptionPrice === 0 ? "#34d399" : "#fbbf24" }}>
                    ...
                    <span className="text-[13px] text-muted-foreground">{p.pricingModel}</span>
                  </div>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 py-2 border-b border-border flex items-center gap-2">
        ...
        <span className="text-[15px] text-muted-foreground">— a new economic actor: universities, NGOs, startups, governments</span>
        <span className="ml-auto text-[14px] text-muted-foreground">{developers.length} developers</span>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto gdt-scroll p-3 space-y-1.5">
        {loading && <div className="flex h-32 items-center justify-center"><Loader2 className="size-5 animate-spin text-primary" /></div>}
        {developers.map((d, i) => {
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-[16px] font-medium truncate">{d.name}</span>
                    ...
                    {d.countryCode && <span className="text-[13px] text-muted-foreground">{d.countryCode}</span>}
                    <span className="rounded px-1 py-0.5 text-[13px] font-medium capitalize" style={{ color: d.certificationLevel === "official" ? "#f43f5e" : d.certificationLevel === "certified" ? "#34d399" : "#fbbf24", background: `${d.certificationLevel === "official" ? "#f43f5e" : d.certificationLevel === "certified" ? "#34d399" : "#fbbf24"}1a` }}>{d.certificationLevel}</span>
                  </div>
                  {d.bio && <p className="text-[14px] text-muted-foreground line-clamp-1 mb-0.5">{d.bio}</p>}
                ...
                <div className="text-right shrink-0">
                  ...
                  <div className="text-[13px] text-muted-foreground">IC revenue</div>
                </div>
              </div>
            </div>
          );
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 py-2 border-b border-border flex items-center gap-2">
        ...
        <span className="text-[15px] text-muted-foreground">— complete intelligence systems (outcome-based, not capability-based)</span>
        <span className="ml-auto text-[14px] text-muted-foreground">{solutions.length} solutions</span>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto gdt-scroll p-3 space-y-1.5">
        {loading && <div className="flex h-32 items-center justify-center"><Loader2 className="size-5 animate-spin text-primary" /></div>}
        {solutions.map((s) => (
          <div key={s.solutionId} className="rounded-lg border border-border bg-card/40 px-3 py-2.5">
            <div className="flex items-stretch gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span className="font-mono text-[14px] font-semibold text-muted-foreground">{s.solutionId}</span>
                  <span className="rounded bg-cyan-500/10 px-1 py-0.5 text-[13px] font-medium text-cyan-300">{s.problemCategory}</span>
                  <span className="rounded px-1 py-0.5 text-[13px] font-semibold" style={{ color: LIFECYCLE_META[s.lifecycleStage]?.color ?? "#a1a1aa", background: `${LIFECYCLE_META[s.lifecycleStage]?.color ?? "#a1a1aa"}1a` }}>{LIFECYCLE_META[s.lifecycleStage]?.label ?? s.lifecycleStage}</span>
                  <span className="ml-auto text-[13px] text-muted-foreground">{timeAgo(s.publishedAt)}</span>
                </div>
                <div className="text-[17px] font-medium mb-0.5">{s.displayName}</div>
                ...
                <div className="flex items-center gap-1 flex-wrap mb-1.5">
                  ...
                  {s.componentPackageIds.map((pid: string) => (
                    <span key={pid} className="rounded bg-cyan-500/10 px-1 py-0.5 text-[13px] font-mono text-cyan-300">{pid}</span>
                  ))}
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[16px] font-bold font-mono text-amber-400">{s.price.toLocaleString()} IC</span>
                  <span className="text-[13px] text-muted-foreground">{s.pricingModel}</span>
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 py-2 border-b border-border flex items-center gap-2">
        ...
        <span className="text-[15px] text-muted-foreground">— who uses what, what solves what, what produces what</span>
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 py-2 border-b border-border flex items-center gap-2">
        ...
        <span className="text-[15px] text-muted-foreground">— ratings + quality assessments from consumers</span>
        <span className="ml-auto text-[14px] text-muted-foreground">{reviews.length} reviews</span>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto gdt-scroll p-3 space-y-1.5">
        {loading && <div className="flex h-32 items-center justify-center"><Loader2 className="size-5 animate-spin text-primary" /></div>}
        {reviews.map((r) => (
          <div key={r.reviewId} className="rounded-lg border border-border bg-card/40 px-3 py-2.5">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <span className="font-mono text-[14px] font-semibold text-muted-foreground">{r.reviewId}</span>
              <span className="text-[14px] text-muted-foreground">package: <span className="font-mono">{r.packageId}</span></span>
              <span className="ml-auto text-[13px] text-muted-foreground">{timeAgo(r.reviewedAt)}</span>
            </div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[15px] font-medium">{r.reviewerName}</span>
              <span className="rounded bg-foreground/5 px-1 py-0.5 text-[13px] text-muted-foreground capitalize">{r.reviewerType}</span>
            ...
            {r.reviewText && <p className="text-[15px] text-foreground/70 italic mb-1">"{r.reviewText}"</p>}
