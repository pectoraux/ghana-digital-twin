    ...
    <div className="flex h-full w-full flex-col">
      <div className="border-b border-border px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <span className="rounded-md bg-primary/10 border border-primary/20 px-1.5 py-0.5 text-[14px] font-mono text-primary">package: autonomous-organizations</span>
          <p className="text-[15px] text-muted-foreground hidden lg:block">Digital institutions native to the intelligence economy</p>
        </div>
  ...
  return (
    <div className="h-full overflow-y-auto gdt-scroll p-4 space-y-4">
        ...
        <div className="rounded-lg border border-border bg-card/40 p-4">
          ...
          <div className="space-y-1">
            {o.organizations.map((org: any) => {
              ...
              return (
                <div key={org.orgId} className="flex items-center gap-2 text-[15px] py-1">
        ...
        <div className="rounded-lg border border-border bg-card/40 p-4">
          ...
          <div className="space-y-1">
            {o.recentTransactions.map((t: any) => {
              ...
              return (
                <div key={t.transactionId} className="flex items-center gap-2 text-[15px] py-1">
      ...
      <div className="rounded-lg border border-primary/20 bg-primary/5 p-4">
        <div className="flex items-center gap-2 mb-2"><Building2 className="size-4 text-primary" /><span className="text-[15px] font-semibold text-primary">AUTONOMOUS INTELLIGENCE ORGANIZATIONS</span></div>
  ...
  return (
    <div className="h-full flex">
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="px-4 py-2 border-b border-border flex items-center gap-2"><Building2 className="size-4 text-emerald-400" /><span className="text-xs font-semibold">Organizations</span><span className="ml-auto text-[14px] text-muted-foreground">{orgs.length} orgs</span></div>
                ...
                <div className="flex items-stretch gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className="font-mono text-[14px] font-semibold text-muted-foreground">{org.orgId}</span>
                      ...
                      <span className="rounded px-1 py-0.5 text-[13px] font-semibold" style={{ color: tier.color, background: `${tier.color}1a` }}>{tier.label}</span>
                      <span className={cn("rounded px-1 py-0.5 text-[13px]", org.status === "active" ? "bg-emerald-500/10 text-emerald-300" : "bg-rose-500/10 text-rose-300")}>{org.status}</span>
                      <span className="ml-auto font-mono text-[16px] font-bold text-amber-400">{(org.treasuryBalance / 1000).toFixed(0)}K IC</span>
                    </div>
                    <div className="text-[16px] font-medium mb-0.5">{org.displayName}</div>
                    <p className="text-[15px] text-foreground/70 leading-snug line-clamp-1 mb-1">{org.mission}</p>
      ...
      <div className="hidden w-[340px] shrink-0 border-l border-border bg-card/20 lg:flex flex-col">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto gdt-scroll p-3 space-y-3">
          {detail ? (<>
            <div><SectionLabel className="mb-1">Mission</SectionLabel><p className="text-[15px] text-foreground/80">{detail.mission}</p></div>
            {detail.treasury && (
              <div><SectionLabel className="mb-1">Treasury</SectionLabel>
                ...
                <div className="text-[14px] text-muted-foreground">income: {detail.treasury.totalIncome.toLocaleString()} · expenses: {detail.treasury.totalExpenses.toLocaleString()}</div>
              </div>
            )}
              ...
              <div><SectionLabel className="mb-1">Employed Agents ({detail.agentContracts.length})</SectionLabel>
                <div className="space-y-1">{detail.agentContracts.map((c: any) => (
                  <div key={c.contractId} className="rounded border border-border bg-card/30 px-2 py-1.5">
                    <div className="flex items-center gap-2"><Brain className="size-3 text-violet-400" /><span className="text-[15px] font-medium flex-1 truncate">{c.agentName}</span></div>
                    <div className="text-[13px] text-muted-foreground mt-0.5">role: {c.role} · budget: {c.budgetAllocation.toLocaleString()} IC/mo</div>
                  </div>
                ))}</div>
              </div>
            )}
                ...
                <div className="space-y-1">{detail.objectives.map((obj: any) => {
                  ...
                  return (
                    <div key={obj.objectiveId} className="rounded border border-border bg-card/30 px-2 py-1.5">
                      <div className="flex items-center gap-2 mb-1"><span className="text-[15px] font-medium flex-1 truncate">{obj.title}</span><span className="rounded px-1 py-0.5 text-[13px] font-semibold" style={{ color: smeta.color, background: `${smeta.color}1a` }}>{smeta.label}</span></div>
                      <ConfidenceBar value={obj.progressPercent / 100} showLabel={false} size="sm" />
                      <div className="text-[13px] text-muted-foreground mt-0.5">{obj.currentValue}/{obj.targetValue}{obj.unit} · {obj.progressPercent.toFixed(0)}%</div>
                    </div>
                  );
                })}</div>
              </div>
            ...
            {detail.charter && (
              <div><SectionLabel className="mb-1">Charter ({detail.charter.ruleCount} rules)</SectionLabel>
                <p className="text-[14px] text-muted-foreground italic mb-1">{detail.charter.preamble.slice(0, 120)}...</p>
                <div className="space-y-0.5">{detail.charter.rules.map((r: any, i: number) => (
                  <div key={i} className="text-[14px] text-foreground/70">#{r.number}: {r.title}</div>
                ))}</div>
              </div>
            )}
          </>) : <div className="text-[15px] text-muted-foreground">Select an organization</div>}
        </div>
      </div>
    </div>
  );
...
function TreasuryTab() {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 py-2 border-b border-border flex items-center gap-2"><Wallet className="size-4 text-amber-400" /><span className="text-xs font-semibold">Treasury Transactions</span><span className="text-[15px] text-muted-foreground">— institutional economics</span><span className="ml-auto text-[14px] text-muted-foreground">{txs.length} transactions</span></div>
      <div className="min-h-0 flex-1 overflow-y-auto gdt-scroll p-3 space-y-1.5">
        {loading && <div className="flex h-32 items-center justify-center"><Loader2 className="size-5 animate-spin text-primary" /></div>}
        {txs.map((t) => {
          const meta = TX_TYPE_META[t.type] ?? TX_TYPE_META.income;
          return (
            <div key={t.transactionId} className="rounded-lg border border-border bg-card/40 px-3 py-2 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2"><span className="font-mono text-[14px] font-semibold text-muted-foreground">{t.transactionId}</span><span className="rounded px-1 py-0.5 text-[13px]" style={{ color: meta.color, background: `${meta.color}1a` }}>{t.category}</span></div>
                <div className="text-[15px] text-foreground/70 truncate">{t.description}</div>
                <div className="text-[13px] text-muted-foreground">org: {t.orgId} {t.counterpartyName ? `· ${t.counterpartyName}` : ""}</div>
              </div>
              <div className="text-right shrink-0">
                <div className="font-mono text-[18px] font-bold" style={{ color: meta.color }}>{t.amount > 0 ? "+" : ""}{t.amount.toLocaleString()}</div>
                <div className="text-[13px] text-muted-foreground">balance: {t.balanceAfter.toLocaleString()}</div>
              </div>
            </div>
          );
        })}
...
function ObjectivesTab() {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 py-2 border-b border-border flex items-center gap-2"><Target className="size-4 text-rose-400" /><span className="text-xs font-semibold">Organization Objectives</span><span className="text-[15px] text-muted-foreground">— goals with metrics + progress</span><span className="ml-auto text-[14px] text-muted-foreground">{objectives.length} objectives</span></div>
      <div className="min-h-0 flex-1 overflow-y-auto gdt-scroll p-3 space-y-1.5">
        {loading && <div className="flex h-32 items-center justify-center"><Loader2 className="size-5 animate-spin text-primary" /></div>}
        {objectives.map((obj) => {
          const smeta = OBJ_STATUS_META[obj.status] ?? OBJ_STATUS_META.active;
          return (
            <div key={obj.objectiveId} className="rounded-lg border border-border bg-card/40 px-3 py-2.5">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                <span className="font-mono text-[14px] font-semibold text-muted-foreground">{obj.objectiveId}</span>
                <span className="rounded px-1 py-0.5 text-[13px] font-semibold" style={{ color: smeta.color, background: `${smeta.color}1a` }}>{smeta.label}</span>
                <span className="text-[14px] text-muted-foreground">org: {obj.orgId}</span>
              </div>
              <div className="text-[16px] font-medium mb-1">{obj.title}</div>
              <p className="text-[15px] text-foreground/70 leading-snug line-clamp-2 mb-1.5">{obj.description}</p>
...
function ServicesTab() {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 py-2 border-b border-border flex items-center gap-2"><Store className="size-4 text-cyan-400" /><span className="text-xs font-semibold">Organization Services</span><span className="text-[15px] text-muted-foreground">— intelligence as a service</span><span className="ml-auto text-[14px] text-muted-foreground">{services.length} services</span></div>
      <div className="min-h-0 flex-1 overflow-y-auto gdt-scroll p-3 space-y-1.5">
        {loading && <div className="flex h-32 items-center justify-center"><Loader2 className="size-5 animate-spin text-primary" /></div>}
        {services.map((s) => {
          ...
          return (
            <div key={s.serviceId} className="rounded-lg border border-border bg-card/40 px-3 py-2.5">
              <div className="flex items-stretch gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="font-mono text-[14px] font-semibold text-muted-foreground">{s.serviceId}</span>
                    <span className="rounded px-1 py-0.5 text-[13px] font-semibold" style={{ color: meta.color, background: `${meta.color}1a` }}>{s.serviceType}</span>
                    <span className="text-[14px] text-muted-foreground">by {s.orgName}</span>
                    <span className="ml-auto font-mono text-[18px] font-bold text-amber-400">{s.price.toLocaleString()} IC/{s.pricingModel === "subscription" ? "mo" : "use"}</span>
                  </div>
                  <div className="text-[16px] font-medium mb-0.5">{s.name}</div>
                  <p className="text-[15px] text-foreground/70 leading-snug line-clamp-2 mb-1.5">{s.description}</p>
                  {s.includes.length > 0 && (
                    <div className="flex items-center gap-1 flex-wrap">
                      <span className="text-[13px] text-muted-foreground">includes:</span>
                      {s.includes.map((p: string) => <span key={p} className="rounded bg-cyan-500/10 px-1 py-0.5 text-[13px] font-mono text-cyan-300">{p}</span>)}
                    </div>
                  )}
                </div>
              </div>
