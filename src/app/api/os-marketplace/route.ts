// GET /api/os-marketplace — overview + packages
// POST /api/os-marketplace { action: "seed" }

import { NextRequest, NextResponse } from "next/server";
import { getOSOverview } from "@/lib/os-marketplace/engine";
...
export async function GET() {
  const [overview, pkg] = await Promise.all([
    getOSOverview(),
    db.oSPackage.findMany({ where: { active: true } }),
  ]);
...
export async function POST(req: NextRequest) {
  ...
  if (body.action === "seed") {
    const result = await seedOSMarketplace();
